<?php
//https://code.google.com/p/php-form-builder-class/source/browse/trunk/includes/class.spreadsheet.php
		...
		if(!empty($this->token)) {
			...
			if(!empty($url)) {
				...
				if($columnIDs) {
					...
					$status = curl_getinfo($curl, CURLINFO_HTTP_CODE);
					curl_close($curl);
					$this->debug["AddResult"] = $response;
				}else{
					$this->debug["Columns"] = array();
				}
			}else{
				$this->debug["SheetURL"] = "Failed to retrieve, please double check the spreadsheet and worksheet names.";
			}
		}
	}
