...
$db = JFactory::getDBO();
$helpurl = '//www.youtube-nocookie.com/embed/LK1aCGWMhyc?list=PLBC9B0C0BFE98657E';
...
window.addEvent('load', function (e){
	...
	if(j30option){
		...
		if(j32compat){
			var t3tplopt = new Element('li', {'html': '<a href="#t3tplopt" data-toggle="tab">T3 Options</a>'}).inject(j30option.getElements('li')[0], 'after');
			new Element('div', {'id': 't3tplopt', 'class': 'tab-pane'}).inject(j30option.getNext().getElements('div')[0], 'after').adopt($('ja-tabswrapmain'));

			var extra = $('attrib-EXTRA_SETTING');
			if(extra && !extra.getElements('input, select, textarea').length){
				//hide
				$('attrib-EXTRA_SETTING').setStyle('display', 'none');
				j30option.getElements('a').each(function(el){
					if((typeof el.hash != 'undefined' ? el.hash : el.href.replace(/.*(?=#[^\s]*$)/, '')) == '#attrib-EXTRA_SETTING'){
						$(el).getParent().setStyle('display', 'none');
					}
				});
			}

			$(t3tplopt).addEvent('click', function(){
				setTimeout(function(){
					jatabs.resize();
				},10);
			});
